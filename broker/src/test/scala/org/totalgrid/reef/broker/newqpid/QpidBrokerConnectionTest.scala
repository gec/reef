...
class QpidBrokerConnectionTest extends FunSuite with ShouldMatchers {
  ...
  val defaults = BrokerConnectionInfo.loadInfo("test")
  val defaultTimeout = 5000
  def fixture(config: BrokerConnectionInfo)(test: BrokerConnection => Unit): Unit = {
  ...
  test("Connection Timeout") {
    val config = new BrokerConnectionInfo("127.0.0.1", 10000, "", "", "")
