...
class Promise[A](private var result: Option[A]) extends IPromise[A] {
  ...
  final override def isComplete = result.isDefined

  def onResponse(value: A) = mutex.synchronized {
    result match {
      case Some(x) =>
  ...
  final override def await: A = mutex.synchronized {
    result match {
      case None =>
        mutex.wait()
        result.get
  ...
  final override def listen(fun: A => Unit) = mutex.synchronized {
    result match {
      case Some(x) => fun(x)
