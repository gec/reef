...
import org.totalgrid.reef.proto.Communications.{ ChannelStatus => ChannelStatusProto}

import org.totalgrid.reef.services.framework.{BasicSyncModeledService, ServiceTransactable, BasicModelFactory, SquerylServiceModel, EventedServiceModel}
...
class ChannelStatusService(protected val modelTrans: ServiceTransactable[PointServiceModel])
    extends BasicSyncModeledService[ChannelStatusProto, ChannelStatus, PointServiceModel] {
...
class ChannelStatusServiceModel(protected val subHandler: ServiceSubscriptionHandler)
   ...
   */
  override def getEventProtoAndKey(entry: ChannelStatus) = {
