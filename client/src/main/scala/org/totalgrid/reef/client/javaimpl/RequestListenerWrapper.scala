...
class RequestListenerWrapper(listener: RequestListener) extends RequestSpy {

  def onRequestReply[A](verb: Verb, request: A, response: Future[Response[A]]) {
    val promise = FuturePromise(response.map(ResponseWrapper.convert(_)))
    listener.onRequest(verb, request, promise)
  }
}
