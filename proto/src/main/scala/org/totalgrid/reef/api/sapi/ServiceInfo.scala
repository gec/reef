...
object ServiceInfo {
  ...
  def apply[A](descriptor: TypeDescriptor[A]): ServiceInfo[A, A] =
    ServiceInfo[A, A](descriptor, descriptor, TypeDescriptorHelpers.getEventExchange(descriptor))

  def apply[A, B](descriptor: TypeDescriptor[A], subDescriptor: TypeDescriptor[B]): ServiceInfo[A, B] =
    ServiceInfo[A, B](descriptor, subDescriptor, TypeDescriptorHelpers.getEventExchange(subDescriptor))
