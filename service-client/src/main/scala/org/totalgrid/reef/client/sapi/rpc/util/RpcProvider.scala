...
import org.totalgrid.reef.client.{ ServiceProviderFactory, ServiceProviderInfo, Client }
...
object RpcProvider {
  ...
  def apply(fun: (Client) => AnyRef, interfaces: List[Class[_]]): ServiceProviderInfo = {
    new ServiceProviderInfo {
      ...
      def getFactory = new ServiceProviderFactory {
        def createRpcProvider(client: Client): AnyRef = fun(client)
      }
    }
  }
}