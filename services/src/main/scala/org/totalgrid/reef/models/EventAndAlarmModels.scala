...
case class EventStore(
  ...
  val groups = LazyVar(entityId.map { x => EntityQuery.getParentOfType(x, "owns", "EquipmentGroup").toList }.getOrElse(Nil))
  val equipments = LazyVar(entityId.map { x => EntityQuery.getParentOfType(x, "owns", "Equipment").toList }.getOrElse(Nil))
}
