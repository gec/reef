...
class CommandTestRig(dbConnection: DbConnection) {
  ...
  val defaultContextSource = new MockRequestContextSource(new ServiceDependenciesDefaults(dbConnection))

  def transaction[A](f: RequestContext => A) = defaultContextSource.transaction(f)
  ...
  def seed(context: RequestContext, name: String): Command = {
...
class CommandLockServiceModelTest extends DatabaseUsingTestBase with RunTestsInsideTransaction with SyncServicesTestHelpers {
  ...
  test("Block") {
    ...
    r.transaction { context =>
      val cmd1 = r.seed(context, "cmd01")
      val cmd2 = r.seed(context, "cmd02")
