...
class LoaderServicesImpl(client: Client) extends ApiBase(client) with LoaderServices with AllScadaServiceImpl {
  def addEquipment(eq: Entity) = ops.operation("Can't add equipment: " + eq.getUuid.getUuid + " name: " + eq.getName) {
    _.put(eq).map { _.one }
  }

  def addPoint(point: Point) = ops.operation("Can't add point: " + point.getUuid.getUuid + " name: " + point.getName) {
    _.put(point).map { _.one }
  }

  def addCommand(cmd: Command) = ops.operation("Can't add command: " + cmd.getUuid.getUuid + " name: " + cmd.getName) {
    _.put(cmd).map { _.one }
  }

  def addConfigFile(eq: ConfigFile) = ops.operation("Can't add configFile: " + eq.getUuid.getUuid + " name: " + eq.getName) {
    _.put(eq).map { _.one }
  }

  def addEndpoint(eq: CommEndpointConfig) = ops.operation("Can't add endpoint: " + eq.getUuid.getUuid + " name: " + eq.getName) {
    _.put(eq).map { _.one }
  }

  def addChannel(eq: CommChannel) = ops.operation("Can't add channel: " + eq.getUuid.getUuid + " name: " + eq.getName) {
    _.put(eq).map { _.one }
  }
  ...
  def getFeedbackCommands(uuid: ReefUUID) = {
    ops.operation("Couldn't get feedback commands for point: " + uuid.getUuid) { session =>
