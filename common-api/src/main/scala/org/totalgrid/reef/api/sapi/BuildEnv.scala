...
object BuildEnv {
   ...
   */
  def environment = Option(System.getenv("RAILS_ENV")) getOrElse "development"

  /**
   * looks for a system property that has the same name as the file or returns the default
   */
  def path(name: String) = Option(System.getenv(name)) getOrElse configPath + name

  def cfgName(base: String, subPid: String, env: String): String = env match {
   ...
   */
  def configPath = Option(System.getProperty("config.path")) getOrElse "../"
