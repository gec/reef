...
def processLines(opt_file, deserializers, lines)
  ...
  ns = "org.totalgrid.reef.client.service.proto"
  cn = false
  lines.each do |line|
    if starts_with?(line, "//")
...
def generatePackageInfo(fname, file_text)
  ...
  dir_name = "./service-client/src/main/java/org/totalgrid/reef/client/service/protodoc/#{proto_name.downcase}"
...
f = File.open(File.join(File.dirname(__FILE__),"./service-client/src/main/scala/org/totalgrid/reef/client/service/proto/OptionalProtos.scala"), 'wb')

types = %w[Application Commands FEP Mapping Measurements ProcessStatus Alarms Events Processing Model Auth]

scala_imports = types.collect{|t| "import org.totalgrid.reef.client.service.proto.#{t}._"}.join("\n")
...
deseralizers = File.open(File.join(File.dirname(__FILE__),"./service-client/src/main/scala/org/totalgrid/reef/client/service/proto/Descriptors.scala"), 'wb')
...
Dir[File.join(File.dirname(__FILE__),"./service-client/src/proto/*.proto")].each do |files|
  files.each do |fname|
    all_names << fname
  end
end
