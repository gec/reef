...
trait InteractionRecorder extends RequestSpy {
  ...
  def onRequestReply[A](verb: Envelope.Verb, request: A, future: Future[Response[A]]) {

    if (explanations.nonEmpty) {
      explainedRequests ::= Documenter.RequestWithExplanation(explanations.dequeue, verb, request, future.await)
    }
