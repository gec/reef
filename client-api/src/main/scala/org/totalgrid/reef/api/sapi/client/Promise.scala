...
object Promise {
  def from[A](future: Future[Result[A]]): Promise[A] = new Promise(future)
}

class Promise[+A](future: Future[Result[A]]) {
  ...
  //def flatMap[B](fun: A => Promise[B]) = Promise.from(future.flatMap(_.map(fun(_).future)))
