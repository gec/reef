...
import org.totalgrid.reef.sapi.{ Destination, BasicRequestHeaders }
...
class OrderedServiceTransmitterTest extends FunSuite with ShouldMatchers {
  ...
  class QueuedResponseClientSession extends ClientSession {
    ...
    final override def request[A](verb: Verb, payload: A, env: BasicRequestHeaders, destination: Destination): Promise[Response[A]] = queue.synchronized {
      numRequests += 1
