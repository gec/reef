...
abstract class AMQPPublisher(exchangeList: List[String] = Nil, bufferSize: Int = AMQPPublisher.defaultBufferSize) extends ChannelObserver with BrokerChannelCloseListener with Executor with Logging {
    ...
    this.execute {
      if (channel.isDefined) {
        try {
          ...
          case e: Exception =>
            logger.error("failed to publish message, exchange: " + exchange + ", key: " + key + ", error: " + e.getMessage, e)
  ...
  private def delayMessage(b: Array[Byte], exchange: String, key: String) {
    if (bytesDelayed + b.size < bufferSize) {
      ...
      logger.error("unable to queue msg, buffer size exceeded: exchange: " + exchange + ", key: " + key)
    }
  }
