...
class AmqpClientSession(
  ...
  final override def asyncRequest[A](verb: Envelope.Verb, request: A, headers: BasicRequestHeaders, dest: Destination)(callback: Response[A] => Unit) {
    ...
    headers.merge(getHeaders).toEnvelopeRequestHeaders.foreach(requestBuilder.addHeaders)
